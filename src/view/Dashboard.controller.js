...
sap.ui.define([
  ...
  function(jQuery, Controller, JSONModel, FeedItem, FlattenedDataset) {
    ...
    Dashboard.prototype.runPageJellyBeans = function(oNavContainer) {
      ...
      // Send a tweet.
      this.postTweet(this._mUpdate.getProperty("/request_id"));
    };
     ...
     * @param  {String} sId Survey request id
     */
    Dashboard.prototype.postTweet = function(sId) {
      jQuery.ajax({
        url: '/twitter/post/' + sId,
        type: 'GET',
        async: true,
        success: jQuery.proxy(function(oData, mResponse) {
          //sap.m.MessageToast.show("Tweeted");
        }, this),
        error: jQuery.proxy(function(mError) {
          sap.m.MessageToast.show("Error tweeting");
        }, this)
      });
    };
